...
---
= Bekannte Einschränkungen bei NetApp Backup and Recovery für SQL Server-Workloads
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Diese Einschränkungen sollten vor der Verwendung von NetApp Backup and Recovery für SQL Server Workloads geprüft werden.
...
Sie können einer Ressource nicht mehrere Richtlinien zuweisen, die dieselbe Zeitplanstufe verwenden (z. B. stündlich, täglich oder wöchentlich). NetApp Backup and Recovery wird diese Ressourcen nicht aus SnapCenter importieren.
...
Wenn eine SnapCenter-Richtlinie sowohl im Tages- als auch im Monatsintervall wiederholt wird, wird die Richtlinie von NetApp Backup and Recovery nicht importiert.

Beispielsweise kann einer Ressource nicht sowohl eine Tagesrichtlinie (mit einem Intervall von 7 Tagen oder weniger oder mehr als 7 Tagen) als auch eine Monatsrichtlinie zugewiesen werden.
...
NetApp Backup and Recovery importiert keine On-Demand-Backup-Richtlinien von SnapCenter.
...
SnapCenter ordnet Speichercluster oder Speicher-VMs für Ressourcen nicht Hosts zu, NetApp Backup and Recovery hingegen schon. Das lokale ONTAP-Cluster oder die Speicher-VM wird in den Preview-Versionen von NetApp Backup and Recovery keinem Host zugeordnet. Außerdem unterstützt die NetApp Console keine Speicher-VMs.
