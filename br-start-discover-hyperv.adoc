...
.. *Hyper-V-Einstellungen*: Folgendes ist zu tun:
+
...
... Ein neuer Hyper-V-Host wird hinzugefügt, indem der FQDN oder die IP-Adresse des Hosts eingegeben und ein Console-Agent sowie Anmeldeinformationen ausgewählt werden. Wenn der FQDN vom Console-Agent nicht aufgelöst werden kann, ist stattdessen die IP-Adresse zu verwenden. Für Failover-Cluster ist die Clusterverwaltungs-IP-Adresse einzugeben.


.. *Erweiterte Einstellungen*: Optional kann ein Installationspfad für das NetApp Hyper-V Plug-in eingegeben werden.
