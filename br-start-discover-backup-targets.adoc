...
[role="lead"]
Offsite-Backup-Ziele wie AWS S3, Azure Blob Storage, Google Cloud Storage oder StorageGRID können erkannt oder manuell hinzugefügt werden, sodass Backups außerhalb der primären Umgebung gespeichert werden können.
...
.Schritte
. Im Menü NetApp Backup and Recovery die Option *Backup-Ziele* auswählen.
. Wählen Sie *Sicherungsziel ermitteln*.
. Wählen Sie einen der verfügbaren Backup-Zieltypen aus: *AWS*, *Azure*, *StorageGRID*, *ONTAP S3* oder *GCP*.
. Wählen Sie *Weiter*.
. (Nur für AWS- und Azure-Backup-Ziele) Wählen Sie im Abschnitt *Speicherort der Anmeldeinformationen auswählen* den Speicherort aus, an dem sich die Anmeldeinformationen befinden, und wählen Sie dann aus, wie die Anmeldeinformationen verknüpft werden sollen.
. Wählen Sie *Weiter*.
...
.Schritte
. Im Menü NetApp Backup and Recovery die Option *Backup-Ziele* auswählen.
. Wählen Sie ein Ziel aus und wählen Sie rechts das Symbol image:icon-action.png["Symbol „Aktionen“"] *Aktionen* und dann *Bucket hinzufügen* aus.
...
.Schritte
. Im Menü NetApp Backup and Recovery die Option *Backup-Ziele* auswählen.
. Wählen Sie ein Ziel aus und wählen Sie rechts das Symbol image:icon-action.png["Symbol „Aktionen“"] *Aktionen* und dann *Anmeldeinformationen ändern* aus.
