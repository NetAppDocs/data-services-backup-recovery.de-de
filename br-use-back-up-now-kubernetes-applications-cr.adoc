...
summary: 'Benutzerdefinierte Ressourcen (CRs) ermöglichen die Erstellung von Snapshots und Backups einer Kubernetes-Anwendung auf Abruf, wodurch eine Baseline festgelegt oder die neuesten Datenänderungen geschützt werden.' 
---
= Kubernetes-Anwendungen können jetzt mithilfe benutzerdefinierter Ressourcen in NetApp Backup and Recovery gesichert werden.
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Benutzerdefinierte Ressourcen (CRs) ermöglichen die Erstellung von Snapshots und Backups einer Kubernetes-Anwendung auf Abruf, wodurch eine Baseline festgelegt oder die neuesten Datenänderungen geschützt werden.
...
** *spec.reclaimPolicy*: (_Optional_) Definiert, was mit dem AppArchive eines Snapshots geschieht, wenn der Snapshot-CR gelöscht wird. Auch wenn diese Option auf  `Retain` gesetzt ist, wird der Snapshot-CR selbst gelöscht; nur das AppArchive bleibt erhalten. Gültige Optionen:
+
*** `Retain` (Standard)
*** `Delete`
+
...
| protect.trident.netapp.io/pvc-bind-timeout-sec | Zeichenkette | Maximale Wartezeit (in Sekunden), bis neu erstellte PersistentVolumeClaims (PVCs) die  `Bound` Phase erreichen, bevor der Vorgang fehlschlägt. | "1200" (20 Minuten) 
|===