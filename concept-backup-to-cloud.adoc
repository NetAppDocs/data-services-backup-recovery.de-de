...
[role="lead"]
Diese Übersicht zeigt, wie NetApp Backup and Recovery Ihre ONTAP Workloads schützt, welche Workloads und Backup-Ziele unterstützt werden und wie Lizenzierung und Kosten geregelt sind.
...
Die Unterstützung für Backup und Recovery ist in allen ONTAP-Systemen bereits integriert, sodass keine zusätzliche Hardware oder Mediengateways benötigt werden. Dies macht Backup-Vorgänge einfach und kosteneffizient. NetApp Console vereinfacht die Implementierung jeder Backup-Strategie, einschließlich des gesamten Spektrums der 3-2-1-Backup-Varianten, ohne dass mehrere Ressourcenmanager oder spezialisiertes Personal erforderlich sind.
...
** VMware-Workloads werden bei unterstützten Speicheranbietern gesichert.
...
** Siehe link:br-use-oracle-protect-overview.html["Oracle Database-Workloads schützen"].
...
** In Google Cloud wird ein neuer Bucket bereitgestellt, und die  https://cloud.google.com/bigquery["Google Cloud BigQuery-Dienste"^] werden auf Konto-/Projektebene bereitgestellt.
...
* Wenn Sie planen, während der Wiederherstellung von Volume-Daten eine Sicherungsdatei auf Ransomware zu scannen (wenn Sie Datalock und Ransomware-Schutz für Ihre Cloud-Backups aktiviert haben), dann fallen zusätzliche Egress-Kosten von Ihrem Cloud-Anbieter an.
...
* Die dritte Möglichkeit besteht darin, Lizenzen direkt von NetApp zu erwerben. Weitere Informationen finden Sie im <<Lizenzierung>> Abschnitt.
...
* Google Cloud Storage (nur ONTAP Volumes und Kubernetes Workloads)
* StorageGRID
* ONTAP S3
* S3-kompatibler Speicher (nur ONTAP Volumes Workloads)
...
* In Azure verwendet die NetApp Console eine neue oder vorhandene Ressourcengruppe mit einem Speicherkonto für den Blob-Container. Die Console https://docs.microsoft.com/en-us/azure/storage/blobs/anonymous-read-access-prevent["blockiert den öffentlichen Zugriff auf Ihre Blob-Daten"] ist standardmäßig aktiviert.
...
== Bedingungen für NetApp Backup and Recovery
