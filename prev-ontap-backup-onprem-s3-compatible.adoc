...
====
.Schritte
Beachten Sie, dass, falls für die von Ihnen ausgewählten Volumes bereits Snapshot- oder Replikationsrichtlinien angewendet wurden, die später ausgewählten Richtlinien diese bestehenden Richtlinien überschreiben.
...
TIP: Wenn vor der Aktivierung des Snapshots eine benutzerdefinierte Richtlinie erstellt werden soll, kann der System Manager oder der ONTAP CLI `snapmirror policy create`-Befehl verwendet werden. Siehe .
...
** *Vorhandene Snapshots als Sicherungsdateien in den Objektspeicher exportieren*: Falls lokale Snapshots für Volumes in diesem System vorhanden sind, die dem soeben ausgewählten Sicherungsplan-Label entsprechen (z. B. täglich, wöchentlich usw.), wird diese zusätzliche Meldung angezeigt. Durch Aktivieren dieses Kontrollkästchens werden alle historischen Snapshots als Sicherungsdateien in den Objektspeicher kopiert, um den umfassendsten Schutz für Ihre Volumes sicherzustellen.
...
. Optional kann das Kontrollkästchen aktiviert werden, um die Snapshot-Richtlinienbezeichnungen automatisch mit den Replikations- und Sicherungsrichtlinienbezeichnungen zu synchronisieren. Dadurch entstehen Snapshots mit einer Bezeichnung, die den Bezeichnungen in den Replikations- und Sicherungsrichtlinien entspricht. Wenn die Richtlinien nicht übereinstimmen, werden keine Sicherungen erstellt.
