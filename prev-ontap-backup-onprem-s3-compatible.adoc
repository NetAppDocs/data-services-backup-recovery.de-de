...
[role="lead"]
In NetApp Backup und Recovery sind einige Schritte erforderlich, um mit der Sicherung von Volume-Daten Ihrer primären On-Premises ONTAP-Systeme zu beginnen. Backups können an ein sekundäres ONTAP-Speichersystem (ein repliziertes Volume) oder an einen Bucket auf einem S3-kompatiblen Speichersystem oder an beides gesendet werden.
...
*** Geben Sie den Namen der Richtlinie ein.
*** Wählen Sie bis zu fünf Zeitpläne aus, normalerweise mit unterschiedlicher Häufigkeit.
*** Für Backup-to-Object-Richtlinien müssen die Einstellungen für Datalock und Ransomware-Schutz konfiguriert werden. Weitere Informationen zu Datalock und Ransomware-Schutz finden Sie unter link:prev-ontap-policy-object-options.html["Einstellungen der Backup-to-Object-Richtlinie"].
*** Wählen Sie *Erstellen*.
