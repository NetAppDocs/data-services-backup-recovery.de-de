...
[role="lead"]
Mit NetApp Backup und Recovery lassen sich ONTAP Daten mithilfe von „Browse & Restore“ wiederherstellen. Vor der Wiederherstellung sollten der Name des Quellvolumes, das Quellsystem, die Storage-VM und das Datum der Backup-Datei notiert werden. ONTAP Daten können aus einem Snapshot, einem replizierten Volume oder aus Backups, die im Objektspeicher abgelegt sind, wiederhergestellt werden.
...
| NetApp StorageGRID | On-Premises- ONTAP -System  a| 
* On-Premises- ONTAP -System
* Cloud Volumes ONTAP
...
. Wählen Sie *Weiter*.
+
Beachten Sie: Wenn Sie eine Sicherungsdatei im Objektspeicher auswählen und für diese Sicherung der Ransomware-Schutz aktiviert ist (wenn Sie Datalock und Ransomware-Schutz in der Sicherungsrichtlinie aktiviert haben), werden Sie aufgefordert, vor der Datenwiederherstellung einen zusätzlichen Ransomware-Scan der Sicherungsdatei durchzuführen. Wir empfehlen, die Sicherungsdatei auf Ransomware zu scannen. (Für den Zugriff auf die Inhalte der Sicherungsdatei fallen zusätzliche Egress-Kosten Ihres Cloud-Anbieters an.)
...
+
Wenn der Ransomware-Schutz für die Sicherungsdatei aktiv ist (falls Sie Datalock und Ransomware-Schutz in der Sicherungsrichtlinie aktiviert haben), werden Sie aufgefordert, vor der Wiederherstellung der Daten einen zusätzlichen Ransomware-Scan der Sicherungsdatei durchzuführen. Wir empfehlen, die Sicherungsdatei auf Ransomware zu scannen. (Sie werden zusätzliche Egress-Kosten von Ihrem Cloud-Anbieter für den Zugriff auf die Inhalte der Sicherungsdatei haben.)
