...
summary: Mit NetApp Backup and Recovery können Sie ONTAP-Daten mit der Funktion „Browse and Restore“ wiederherstellen. 
---
= ONTAP-Daten mit „Browse & Restore“ in NetApp Backup and Recovery wiederherstellen
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
„Durchsuchen & Wiederherstellen“ wird verwendet, wenn der Name des Quellvolumes, das Quellsystem, die Storage-VM und das Datum der Sicherungsdatei bekannt sind. ONTAP Daten können aus einem Snapshot, einem replizierten Volume oder einer in einem Objektspeicher gespeicherten Sicherung wiederhergestellt werden.
...
NOTE: Die hohe Wiederherstellungspriorität wird beim Wiederherstellen von Daten aus dem Azure-Archivspeicher auf StorageGRID Systeme nicht unterstützt.
...
Es ist möglich, ONTAP Daten aus einer Sicherungsdatei, die sich auf einem sekundären System (einem replizierten Volume) oder im Objektspeicher (einer Sicherungsdatei) befindet, auf die folgenden Systeme wiederherzustellen.
...
| StorageGRID | On-Premises- ONTAP -System  a| 
* On-Premises- ONTAP -System
* Cloud Volumes ONTAP
...
| ONTAP S3 | On-Premises- ONTAP -System  a| 
* On-Premises- ONTAP -System
* Cloud Volumes ONTAP
...
| S3-kompatible Objektspeicher | On-Premises- ONTAP -System  a| 
* On-Premises- ONTAP -System
* Cloud Volumes ONTAP
...
* Für Amazon S3 kann der Console Agent in AWS oder lokal bereitgestellt werden.
* Für Azure Blob kann der Console-Agent in Azure oder lokal bereitgestellt werden.
* Für Google Cloud Storage muss der Konsolenagent in Ihrem Google Cloud Platform VPC bereitgestellt werden
* Für StorageGRID muss der Konsolenagent lokal bereitgestellt werden, mit oder ohne Internetzugang
* Für ONTAP S3 kann der Console-Agent lokal (mit oder ohne Internetzugang) oder in einer Cloud-Provider-Umgebung bereitgestellt werden


Zu beachten ist, dass Verweise auf „On-Premises ONTAP Systeme“ FAS, AFF und ONTAP Select Systeme einschließen.
...
NOTE: Wenn Sie ein SnapLock-fähiges Volume wiederherstellen, bleibt der SnapLock-Schutz nur erhalten, wenn der Zielspeicher WORM-fähig ist. Wenn Sie auf einen nicht-WORM-fähigen Zielspeicher wiederherstellen, wird der SnapLock-Status vom wiederhergestellten Volume entfernt.

Bei der Wiederherstellung eines Cloud-Backups auf einem Cloud Volumes ONTAP System mit ONTAP 9.13.0 oder höher oder auf einem lokalen ONTAP System mit ONTAP 9.14.1 besteht die Möglichkeit, eine _Schnellwiederherstellung_ durchzuführen. Eine Schnellwiederherstellung ist ideal für Disaster-Recovery-Situationen, in denen ein Zugriff auf ein Volume so schnell wie möglich bereitgestellt werden muss. Bei einer Schnellwiederherstellung werden die Metadaten aus der Sicherungsdatei auf ein Volume wiederhergestellt, anstatt die gesamte Sicherungsdatei wiederherzustellen. Eine Schnellwiederherstellung wird für Performance- oder latenztkritische Anwendungen nicht empfohlen und wird für Backups im archivierten Speicher nicht unterstützt.


NOTE: Die Schnellwiederherstellung wird für FlexGroup Volumes nur unterstützt, wenn auf dem Quellsystem, von dem die Cloud-Sicherung erstellt wurde, ONTAP 9.12.1 oder höher ausgeführt wurde. Sie wird für SnapLock Volumes nur unterstützt, wenn auf dem Quellsystem ONTAP 9.11.0 oder höher ausgeführt wurde.
...
. Die Registerkarte *Wiederherstellen* zeigt das Wiederherstellungs-Dashboard an.
...
+
Die Spalte *Speicherort* zeigt an, ob die Sicherungsdatei (Snapshot) *lokal* (ein Snapshot auf dem Quellsystem), *sekundär* (ein repliziertes Volume auf einem sekundären ONTAP System) oder *Objektspeicher* (eine Sicherungsdatei im Objektspeicher) ist. Die gewünschte Datei zur Wiederherstellung kann ausgewählt werden.

. Wählen Sie *Weiter*.
+
Beachten Sie, dass, wenn Sie eine Sicherungsdatei im Objektspeicher auswählen und für diese Sicherung ein Ransomware-Schutz aktiv ist (wenn Sie DataLock und den Ransomware-Schutz in der Sicherungsrichtlinie aktiviert haben), Sie aufgefordert werden, vor der Wiederherstellung der Daten einen zusätzlichen Ransomware-Scan der Sicherungsdatei durchzuführen. Es wird empfohlen, die Sicherungsdatei auf Ransomware zu scannen. (Für den Zugriff auf die Inhalte der Sicherungsdatei fallen zusätzliche Egress-Kosten Ihres Cloud-Anbieters an.)
...
+
** Bei der Wiederherstellung aus Amazon S3 wird der IPspace im ONTAP Cluster ausgewählt, in dem sich das Zielvolume befinden wird, der Zugriffsschlüssel und der geheime Schlüssel für den Benutzer eingegeben, den Sie erstellt haben, um dem ONTAP Cluster Zugriff auf den S3-Bucket zu gewähren, und optional ein privater VPC-Endpunkt für sicheren Datentransfer ausgewählt. Bei Verwendung eines privaten VPC-Endpunkts hat dieser Vorrang vor jeder konfigurierten Proxy-Verbindung.
...
. Optional kann *Diese Wiederherstellung gegenüber der bestehenden Auftragswarteschlange priorisieren* aktiviert werden, um diesem Wiederherstellungsvorgang eine höhere Priorität zuzuweisen. Wenn diese Option aktiviert ist, können andere laufende Aufträge angehalten oder verzögert werden, damit diese Wiederherstellung fortgesetzt werden kann.
. Wählen Sie *Wiederherstellen*.
+
Sie kehren zum Restore Dashboard zurück, sodass der Fortschritt des Restore-Vorgangs überprüft werden kann.
...
* Mit ONTAP 9.15.1 können Sie FlexGroup Ordner mit der Option „Durchsuchen und wiederherstellen“ wiederherstellen.
+
...
+
Wenn der Ransomware-Schutz für die Sicherungsdatei aktiv ist (wenn DataLock und der Ransomware-Schutz in der Sicherungsrichtlinie aktiviert wurden), erscheint eine Aufforderung, vor der Wiederherstellung der Daten einen zusätzlichen Ransomware-Scan der Sicherungsdatei durchzuführen. Es wird empfohlen, die Sicherungsdatei auf Ransomware zu scannen. (Für den Zugriff auf die Inhalte der Sicherungsdatei entstehen zusätzliche Egress-Kosten bei Ihrem Cloud-Anbieter.)
...
+
Beim Wiederherstellen von Ordnern und Dateien stehen verschiedene Optionen für den Speicherort zur Verfügung.

+
** Wenn Sie wie oben gezeigt „Zielordner auswählen“ ausgewählt haben:
+
*** Sie können einen beliebigen Ordner auswählen.
*** Sie können mit dem Mauszeiger über einen Ordner fahren und das Ende der Zeile auswählen, um in die Unterordner zu wechseln und anschließend einen Ordner auswählen.


** Wenn dasselbe Zielsystem und Volume wie der Quellordner oder Speicherort der Quelldatei ausgewählt wurde, kann *Quellordnerpfad beibehalten* ausgewählt werden, um den Ordner oder die Dateien im selben Ordner wiederherzustellen, in dem sie in der Quellstruktur vorhanden waren. Alle gleichen Ordner und Unterordner müssen bereits existieren; Ordner werden nicht erstellt. Beim Wiederherstellen von Dateien am ursprünglichen Speicherort besteht die Möglichkeit, die Quelldateien zu überschreiben oder neue Dateien zu erstellen.
