...
summary: Mit der Such- und Wiederherstellungsfunktion können Sie Volumes, Ordner oder Dateien aus ONTAP Backup-Dateien wiederherstellen. 
---
= ONTAP Daten mit der Such- und Wiederherstellungsfunktion in NetApp Backup and Recovery wiederherstellen
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Mit der Such- und Wiederherstellungsfunktion können Volumes, Ordner oder Dateien aus ONTAP Backups wiederhergestellt werden, wenn der genaue System-, Volume- oder Dateiname nicht bekannt ist. Eine Suche ist über lokale Snapshots, replizierte Volumes und Objektspeicher Backups möglich.
...
* Wenn Sie ein SnapLock-fähiges Volume wiederherstellen, bleibt der SnapLock-Schutz nur erhalten, wenn der Zielspeicher WORM-fähig ist. Wenn Sie auf einen nicht-WORM-fähigen Zielspeicher wiederherstellen, wird der SnapLock-Status vom wiederhergestellten Volume entfernt.


====
Bevor Sie beginnen, sollte eine ungefähre Vorstellung vom Namen oder Speicherort des Volumes oder der Datei bestehen, die wiederhergestellt werden soll.
...
| StorageGRID | On-Premises- ONTAP -System Cloud Volumes ONTAP | On-Premises- ONTAP -System 
...
** Sie müssen eine IPv4-NFS-Daten-LIF auf der Speicher-VM und einen NFS-Export für das Volume konfigurieren. Search & Restore unterstützt sowohl NFS- als auch SMB/CIFS-Volumes.
** Der SnapDiff RPC Server muss auf der Storage-VM aktiviert sein. Die Console übernimmt dies automatisch, wenn die Indizierung auf dem System aktiviert wird. (SnapDiff ist die Technologie, die schnell die Datei- und Verzeichnisunterschiede zwischen Snapshots identifiziert.)
...
NOTE: Backup and Recovery indiziert keine Snapshots, deren Erstellungszeitstempel exakt mit einem anderen Snapshot desselben Volumes übereinstimmt.
...
Zuerst ein separates Volume einbinden, um die Katalogdateien zu speichern. Dies hilft, Datenverlust zu verhindern, falls die Snapshot-Katalogdateien zu groß werden. Dies ist nicht auf jedem Cluster erforderlich; es kann ein beliebiges Volume von einem beliebigen Cluster in Ihrer Umgebung eingebunden werden. Wenn dies nicht erfolgt, funktioniert die Indizierung möglicherweise nicht korrekt.
...
[[mount-volume-steps]]
.Schritt zum Einbinden des Volumes für die Katalogneuindizierung
...
.. Im Bereich „Filtern nach“ die Filterkriterien auswählen. Beispielsweise kann das System, auf dem sich die Daten befinden, und der Dateityp, wie eine .JPEG-Datei, ausgewählt werden. Auch der Typ des Sicherungsspeicherorts kann ausgewählt werden, wenn nur innerhalb verfügbarer Snapshots oder Sicherungsdateien im Objektspeicher gesucht werden soll.
...
+
Beachten Sie, dass die Ergebnisse lokale Volume-Snapshots und replizierte Remote-Volumes identifizieren, die die Datei in Ihrer Suche enthalten. Es besteht die Möglichkeit, die Wiederherstellung aus der Cloud-Backup-Datei, dem Snapshot oder dem replizierten Volume vorzunehmen.
...
+
*** Bei der Wiederherstellung aus Amazon S3 wird der IPspace im ONTAP Cluster ausgewählt, in dem sich das Zielvolume befinden wird. Der Zugriffsschlüssel und der geheime Schlüssel des Benutzers, den Sie erstellt haben, um dem ONTAP Cluster Zugriff auf den S3-Bucket zu gewähren, werden eingegeben. Optional kann ein privater VPC-Endpunkt für sicheren Datentransfer ausgewählt werden. Bei Verwendung eines privaten VPC-Endpunkts hat dieser Vorrang vor jeder konfigurierten Proxy-Verbindung. link:prev-ontap-backup-onprem-aws.html["Details zu diesen Anforderungen anzeigen"].
