...
[role="lead"]
In NetApp Backup und Recovery können Volume-Daten von lokalen ONTAP-Systemen auf ein sekundäres Speichersystem und auf Amazon S3 gesichert werden. Auf dieser Seite lässt sich die Umgebung vorbereiten und Backups aktivieren.
...
NOTE: Wenn Sie für Backup- oder Wiederherstellungs-Datentransfers einen privaten VPC-Endpunkt verwenden, hat dieser Vorrang vor jeder konfigurierten Proxy-Verbindung.
...
NOTE: Die vom Kunden verwaltete Schlüsselverschlüsselung wird nur für Amazon FSx for NetApp ONTAP unterstützt. Für Cloud Volumes ONTAP for AWS wird sie nicht unterstützt.

Siehe  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-setting-up-kms.html["Einrichten der vom Kunden verwalteten Schlüsselverschlüsselung in AWS"^].
