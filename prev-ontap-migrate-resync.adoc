...
[role="lead"]
Mit SnapMirror to Cloud Resync in NetApp Backup und Recovery bleiben vorhandene Cloud-Backups nach der Migration eines Volumes mit SnapMirror Logical Replication (LRSE) weiterhin verfügbar. Diese Funktion ermöglicht die Fortsetzung von Backups ohne erneute Baseline und ist ab ONTAP 9.16.1 verfügbar.

SnapMirror to Cloud Resync unterstützt FlexVol und FlexGroup Migrationen und trägt dazu bei, die Backup-Kontinuität in Hybrid- und Multi-Cloud-Umgebungen aufrechtzuerhalten.
...
Stellen Sie sich vor, Sie haben ein lokales Volume namens Vol1a. Dieses Volume verfügt über drei Snapshots: S1, S2 und S3. Diese Snapshots sind Wiederherstellungspunkte. Vol1a wird mithilfe von SnapMirror to Cloud (SM-C) in der Cloud gesichert, jedoch befinden sich nur S1 und S2 im Objektspeicher.
...
NOTE: Verwenden Sie keine SnapMirror-Richtlinie vom Typ "Vault", um ein Volume von einem alten ONTAP Cluster auf einen neuen ONTAP Cluster oder in einer kaskadierenden Architektur zu klonen. Vault-Richtlinien ändern die Versions-UUID des Snapshots während der Übertragung. Die Versions-UUID des Snapshots wird verwendet, um festzustellen, ob ein gemeinsamer Snapshot zwischen Quelle und Ziel vorhanden ist.
...
+
NetApp Backup and Recovery validiert, dass jedes für die Resynchronisierung ausgewählte Volume einen gemeinsamen Snapshot mit dem Objektspeicher hat. Genauer gesagt muss der neueste im Objektspeicher gespeicherte Snapshot auch im lokalen Snapshot des neu geklonten Volumes vorhanden sein. Diese Prüfung stellt sicher, dass die Volumes für die SnapMirror to Cloud Resync-Operation bereit sind.
...
. Die Eignung der Volumes wird geprüft. Das System prüft, ob die Volumes für die Resynchronisierung geeignet sind. Wenn ein Volume nicht geeignet ist, bedeutet dies, dass die Kriterien für den gemeinsamen Snapshot nicht erfüllt wurden.
+
