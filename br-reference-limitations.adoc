...
[role="lead"]
Diese Einschränkungen sind vor der Verwendung von NetApp Backup and Recovery für ONTAP Volumes zu berücksichtigen.
...
+
Weitere Einzelheiten finden Sie im Blogbeitrag https://community.netapp.com/t5/Tech-ONTAP-Blogs/BlueXP-Backup-and-Recovery-Feature-Blog-May-23-Updates/ba-p/444052["NetApp Backup and Recovery Feature-Blog Mai 2023"^] .
...
* Sie können jeweils nur ein FlexGroup Volume für die Replikation auswählen. Die Sicherung muss für jedes FlexGroup Volume separat aktiviert werden.
+
FlexVol Volumes haben diese Einschränkung nicht: Es können alle FlexVol Volumes im System ausgewählt und denselben Backup-Richtlinien zugewiesen werden.

* Die folgenden Funktionen werden unterstützt in https://docs.netapp.com/us-en/data-services-replication/index.html["NetApp Replication"] , jedoch nicht bei Verwendung der Replikationsfunktion von NetApp Backup and Recovery:
+
...
* Wenn Sie ein FlexVol Volume in ein FlexGroup Volume oder umgekehrt konvertieren, wird das konvertierte Volume als neues Volume betrachtet; Sie müssen nach der Konvertierung einen Schutz für das konvertierte Volume einrichten. Sie können Backups, die vor der Konvertierung erstellt wurden, auf einem anderen Volume wiederherstellen.
...
* Sichern Sie keine SnapMirror Zielvolumes, für die Langzeitaufbewahrungs-Snapshots aktiviert sind. Diese Snapshots werden direkt auf dem Ziel erstellt und nicht in den Objektspeicher verschoben. Diese Einschränkung gilt auch dann, wenn die SnapMirror Richtlinie später geändert wird, um die Langzeitaufbewahrung zu deaktivieren.
...
+
** Beziehungen mit den SnapMirror-Labels `app_consistent` und `all_source_snapshot` werden nicht in der Cloud gesichert.
** Wenn Sie lokale Kopien von Snapshots auf dem SnapMirror Zielvolume erstellen (unabhängig von den SnapMirror Labels), werden diese Snapshots nicht als Backups in die Cloud verschoben. Es ist erforderlich, eine Snapshot-Richtlinie mit den gewünschten Labels auf dem Quell-DP-Volume zu erstellen, damit NetApp Backup and Recovery diese sichern kann.
...
** Die für das Volume angewendete Snapshot-Richtlinie muss eine der von NetApp Backup and Recovery erkannten Richtlinien sein, einschließlich täglich, wöchentlich, monatlich usw. Die Standardrichtlinie „sm_created“ (verwendet für *Mirror All Snapshots*) wird nicht erkannt und das DP Volume wird nicht in der Liste der Volumes angezeigt, die gesichert werden können.
** SVM-DR und Backup und Recovery funktionieren vollständig unabhängig voneinander, wenn das Backup entweder vom Quellsystem oder vom Ziel erstellt wird. Die einzige Einschränkung besteht darin, dass SVM-DR die SnapMirror cloud Beziehung nicht repliziert. Im DR-Szenario, wenn die Storage-VM am sekundären Standort online geht, muss die SnapMirror cloud Beziehung manuell aktualisiert werden.


* MetroCluster -Unterstützung:
+
** Bei Verwendung von ONTAP 9.12.1 GA oder höher wird die Datensicherung unterstützt, wenn eine Verbindung zum primären System besteht. Die gesamte Backup-Konfiguration wird auf das sekundäre System übertragen, sodass Backups in die Cloud nach der Umschaltung automatisch fortgesetzt werden. Es ist nicht erforderlich, die Datensicherung auf dem sekundären System einzurichten (tatsächlich ist dies nicht zulässig).
...
* Ad-hoc-Volume-Backup mit der Schaltfläche *Backup Now* wird auf Datensicherungsvolumes nicht unterstützt.
* SM-BC-Konfigurationen werden nicht unterstützt.
* ONTAP unterstützt keine Verzweigung von SnapMirror-Beziehungen von einem einzelnen Volume zu mehreren Objektspeichern, daher wird diese Konfiguration von NetApp Backup and Recovery nicht unterstützt.
* WORM-/Compliance-Modus auf einem Objektspeicher wird derzeit auf Amazon S3, Google Cloud Platform (GCP), Azure und StorageGRID unterstützt. Dies ist als DataLock-Funktion bekannt und muss über die NetApp Backup and Recovery-Einstellungen verwaltet werden, nicht über die Benutzeroberfläche des Cloud-Anbieters.
...
* Das Wiederherstellen von Ordnern aus lokalen Snapshots wird nicht unterstützt.
* Wiederherstellung von Dateien und Ordnern in einer Disaster-Recovery-Konfiguration (DR): Nach einem Failover behalten Volumes auf einer für DR konfigurierten Storage-VM ihre bestehenden SnapMirror-Beziehungen in einem getrennten Zustand bei. Versuche, Dateien oder Ordner wiederherzustellen, die temporäre SnapMirror-RST-Beziehungen erstellen, werden von ONTAP blockiert, da das Zielvolume bereits eine Beziehung aufweist.
* Mit „Durchsuchen und Wiederherstellen“ können bis zu 100 einzelne Dateien gleichzeitig wiederhergestellt werden.
* Search & Restore kann jeweils nur eine Datei wiederherstellen.
...
== Aktivieren der Indizierung in einer Disaster-Recovery-Konfiguration

Beim Aktivieren der Indizierung auf einem Storage-VM-Cluster in einer Disaster-Recovery-(DR)-Konfiguration erscheint eine Fehlermeldung, die besagt, dass der Vorgang auf einem als Ziel für die Vserver-DR konfigurierten Vserver nicht zulässig ist. Trotz dieser Fehlermeldung setzt der Backend-Prozess den  `snapdiff-rpc-server` Status für die Quell-Storage-VMs auf „Ein“. Nachdem der Storage-VM-DR-Richtlinienzeitplan ausgeführt wurde, wird dieser Status an die Ziel-Storage-VMs weitergegeben.

*Problemumgehung*: Warten Sie, bis der Zeitplan der Speicher-VM-DR-Richtlinie nach dem ersten Fehler ausgeführt wurde, und versuchen Sie dann erneut, die Indizierung zu aktivieren.
