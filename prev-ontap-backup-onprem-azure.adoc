...
[role="lead"]
In NetApp Backup und Recovery können Sie Volumendaten Ihrer lokalen ONTAP-Systeme auf ein sekundäres Speichersystem und in Azure Blob sichern. Auf dieser Seite lässt sich die Umgebung vorbereiten und Backups aktivieren.
...
+
NetApp Backup und Recovery wird in Azure Government-Regionen unterstützt, wenn der Console-Agent in der Cloud bereitgestellt wird, nicht jedoch, wenn er in Ihren Räumlichkeiten installiert ist. Zusätzlich muss der Console-Agent aus dem Azure Marketplace bereitgestellt werden. Der Console-Agent kann in einer Government-Region nicht über die NetApp Console SaaS-Website bereitgestellt werden.
...
+
** Für die PAYGO-Lizenzierung von NetApp Backup und Recovery ist ein Abonnement erforderlich https://marketplace.microsoft.com/en-us/product/netapp.cloud-manager?tab=Overview["NetApp Console Angebot vom Azure Marketplace"^]. Die Abrechnung für NetApp Backup und Recovery erfolgt über dieses Abonnement.
...
* Wenn Sie einen anderen IPspace als den Default verwenden, müssen Sie möglicherweise eine statische Route erstellen, um Zugriff auf den Objektspeicher zu erhalten.
...
+
Zu beachten ist, dass NetApp Backup und Recovery _Azure-Zugriffsrichtlinien_ als Berechtigungsmodell unterstützt. Das Berechtigungsmodell _Azure role-based access control_ (Azure RBAC) wird derzeit nicht unterstützt.
...
+
** Auf der Seite *Systeme* der NetApp Console das System auswählen und im rechten Bereich neben dem Backup und Recovery Service *Aktivieren > Backup Volumes* auswählen.
+
...
+
Die Einführungsseite des Assistenten zeigt die Schutzoptionen, darunter lokale Snapshots, Replikation und Backups. Wenn in diesem Schritt die zweite Option gewählt wurde, erscheint die Seite „Backup-Strategie definieren“ mit einem ausgewählten Volume.
...
NOTE: Wenn Sie *Bring your own bucket* auswählen, stellen Sie sicher, dass der Container, den Sie als Sicherungsziel erstellen, denselben Namen wie das Azure-Speicherkonto hat.
...
. Optional kann das Kontrollkästchen aktiviert werden, um die Snapshot-Richtlinienbezeichnungen automatisch mit den Replikations- und Sicherungsrichtlinienbezeichnungen zu synchronisieren. Dadurch werden Snapshots mit einer Bezeichnung erstellt, die den Bezeichnungen in den Replikations- und Sicherungsrichtlinien entspricht.
