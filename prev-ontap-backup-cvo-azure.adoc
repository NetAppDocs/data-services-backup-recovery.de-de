...
---
= Cloud Volumes ONTAP kann mit NetApp Backup und Recovery in Azure Blob Storage gesichert werden
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
NetApp Backup und Recovery ermöglicht die Sicherung von Volumendaten Ihrer Cloud Volumes ONTAP Systeme in Azure Blob Storage.
...
Sie benötigen außerdem ein Azure-Abonnement für den Speicherplatz, auf dem Ihre Backups gespeichert werden.
...
Um die NetApp Backup und Recovery Search & Restore-Funktionalität nutzen zu können, muss die Rolle für den Console-Agent bestimmte Berechtigungen enthalten, damit auf den Azure Synapse Workspace und das Azure Data Lake Storage-Konto zugegriffen werden kann. Die Berechtigungen sind unten aufgeführt, und die Schritte sind zu befolgen, falls eine Änderung der Richtlinie erforderlich ist.
...
Wählen Sie die Volumes aus, die Sie schützen möchten. Ein geschütztes Volume ist ein Volume, das über eine oder mehrere der folgenden Eigenschaften verfügt: Snapshot-Richtlinie, Replikationsrichtlinie, Backup-to-Object-Richtlinie.
...
NOTE: Wenn Sie ein bestehendes Azure-Speicherkonto ausgewählt haben, sind die Verschlüsselungsinformationen bereits vorhanden, sodass Sie sie jetzt nicht eingeben müssen.
