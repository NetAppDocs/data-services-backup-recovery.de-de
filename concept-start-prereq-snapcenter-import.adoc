...
---
= Voraussetzungen für den SnapCenter-Import für NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Bevor Ressourcen aus dem SnapCenter Plug-in für Microsoft SQL Server in NetApp Backup and Recovery importiert werden, muss sichergestellt sein, dass Ihre Umgebung diese zusätzlichen Anforderungen erfüllt.
...
== Host-Anforderungen für das SnapCenter Plug-in sicherstellen

Um Ressourcen aus dem SnapCenter Plug-in für Microsoft SQL Server zu importieren, muss sichergestellt sein, dass die Host-Anforderungen für die Installation des SnapCenter Plug-in für Microsoft SQL Server erfüllt sind.
...
** `1`: Die UAC-Remote-Beschränkungen sind deaktiviert. Lokale Konten umgehen die Tokenfilterung und verfügen beim Remotezugriff über volle Administratorrechte.
...
----
. Es wird geprüft, ob der aktuelle Wert in  `LocalAccountTokenFilterPolicy` auf „1“ gesetzt ist, indem folgender Befehl ausgeführt wird:
+
[listing]
----
