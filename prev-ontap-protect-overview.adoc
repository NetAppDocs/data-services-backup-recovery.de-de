...
---
= Informationen zum ONTAP Volumeschutz mit NetApp Backup und Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
NetApp Backup und Recovery schützt und stellt Ihre ONTAP Volume-Daten wieder her und archiviert Backups langfristig. Eine 3-2-1-Strategie kann umgesetzt werden, bei der drei Kopien Ihrer Quelldaten auf zwei verschiedenen Speichersystemen und eine Kopie in der Cloud vorhanden sind.
...
Nach der Aktivierung erstellt Backup and Recovery Block-Level-Inkremental-forever-Backups, die auf einem anderen ONTAP Cluster und im Objektspeicher in der Cloud gespeichert werden. Zusätzlich zu Ihrem Quellvolume gibt es:
...
image:diagram-321-overview-unified.png["Ein Diagramm, das zeigt, wie Sicherungsdateien auf dem Quellsystem als Snapshots, als replizierte Volumes auf dem sekundären Speichersystem und als Sicherungsdateien im Objektspeicher existieren."]

NetApp Backup und Recovery verwendet die NetApp SnapMirror Datenreplizierungstechnologie, um Ihre Backups durch das Erstellen von Snapshots und deren Übertragung an die Backup-Speicherorte synchron zu halten.
...
* Die native ONTAP Verschlüsselung während der Übertragung wird mithilfe eines vorab vereinbarten Schlüssels (PSK) zwischen den beiden Systemen eingerichtet.
...
* Im Vergleich zu  https://docs.netapp.com/us-en/data-services-replication/index.html["NetApp Replication"^] umfasst die Replikation in NetApp Backup and Recovery die folgenden Funktionen:
+
...
* Sicherungsdateien werden auf mögliche Ransomware-Angriffe gescannt, und infizierte Backups werden automatisch entfernt oder ersetzt.
...
https://bluexp.netapp.com/cloud-volumes-global-regions["Erfahren Sie mehr mit der globalen Regionskarte"^]
...
NetApp Backup und Recovery ermöglicht das Sichern von ONTAP Volumes von den folgenden Quellsystemen auf die folgenden Sekundärsysteme und Objektspeicher bei öffentlichen und privaten Cloud-Anbietern. Snapshots befinden sich auf dem Quellsystem.


NOTE: Wasabi ist derzeit der einzige qualifizierte S3-kompatible Objektspeicheranbieter. Andere S3-kompatible Anbieter wurden noch nicht validiert.
...
| Cloud Volumes ONTAP in Azure  a| 
...
| Cloud Volumes ONTAP in Google  a| 
...
| S3-kompatibler Objektspeicher | On-Premises- ONTAP -System | On-Premises- ONTAP -System 
|===

NOTE: Verweise auf „On-Premises ONTAP System“ umfassen FAS, AFF und ONTAP Select Systeme.
...
* Wenn Sie planen, während des Wiederherstellungsprozesses von Volume-Daten eine Sicherungsdatei auf Ransomware zu scannen (sofern Sie DataLock und Ransomware-Schutz für Ihre Cloud-Backups aktiviert haben), entstehen Ihnen ebenfalls zusätzliche Kosten für den Datenabfluss von Ihrem Cloud-Anbieter.
...
* _Snapshots_ befinden sich auf dem Quellvolume im Quellsystem.
...
+
** In AWS ermöglicht die Konsole Folgendes: https://docs.aws.amazon.com/AmazonS3/latest/dev/access-control-block-public-access.html["Amazon S3-Funktion „Öffentlichen Zugriff blockieren“"^] auf dem S3-Bucket.
...
NOTE: Die auf das Volume angewendete Snapshot-Richtlinie muss eine der Bezeichnungen aufweisen, die in der Replikationsrichtlinie und der Richtlinie für die Sicherung auf Objekt verwendet werden. Wenn keine übereinstimmenden Bezeichnungen gefunden werden, werden keine Sicherungsdateien erstellt. Wenn beispielsweise „wöchentliche“ replizierte Volumes und Sicherungsdateien erstellt werden sollen, ist eine Snapshot-Richtlinie zu verwenden, die „wöchentliche“ Snapshots erstellt.
...
Wenn Ihr Cluster ONTAP 9.11.1 oder höher verwendet, können Sie Ihre Backups im Objektspeicher vor Löschung und Ransomware-Angriffen schützen. Jede Backup-Richtlinie enthält einen Abschnitt für _DataLock und Ransomware-Schutz_, der auf Ihre Backup-Dateien für einen bestimmten Zeitraum, die _Aufbewahrungsfrist_, angewendet werden kann.
...
Support ist derzeit verfügbar, wenn das Backup-Ziel Amazon S3, Azure Blob oder StorageGRID ist. Weitere Speicheranbieter-Ziele werden in zukünftigen Versionen hinzugefügt.
...
+
Wenn Ihr lokaler Cluster ONTAP 9.12.1 oder höher verwendet und Ihr StorageGRID System Version 11.4 oder höher nutzt, ist das Archivieren älterer Backup-Dateien nach einer bestimmten Anzahl von Tagen in öffentlichen Cloud-Archivspeicher möglich. Aktuell besteht Unterstützung für die Speicherebenen AWS S3 Glacier/S3 Glacier Deep Archive oder Azure Archive. link:prev-ontap-backup-onprem-storagegrid.html["Erfahren Sie mehr über das Archivieren von Backup-Dateien von StorageGRID"].



Siehe link:prev-ontap-policy-object-options.html["Wie das Archivieren älterer Backup-Dateien funktioniert"].
