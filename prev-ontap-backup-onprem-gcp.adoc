...
[role="lead"]
In NetApp Backup und Recovery können Volume-Daten von lokalen ONTAP-Systemen auf ein sekundäres Speichersystem und in Google Cloud Storage gesichert werden. Auf dieser Seite finden Sie Informationen zur Vorbereitung Ihrer Umgebung und zur Aktivierung von Backups.
...
* Beachten Sie, dass Sie, wenn Sie einen anderen IPspace als den Default verwenden, möglicherweise eine statische Route erstellen müssen, um Zugriff auf den Objektspeicher zu erhalten.
...
+
Wenn das Google Cloud Storage-Ziel für Ihre Backups auf der NetApp Console-Seite *Systems* vorhanden ist, kann der ONTAP Cluster auf Google Cloud Storage gezogen werden.
...
** *Verschlüsselungsschlüssel*: Wenn Sie einen neuen Google Cloud Storage-Bucket erstellt haben, geben Sie die vom Anbieter erhaltenen Verschlüsselungsschlüsselinformationen ein. Es besteht die Möglichkeit, entweder die standardmäßigen Google Cloud-Verschlüsselungsschlüssel zu verwenden oder eigene, vom Kunden verwaltete Schlüssel aus dem Google Cloud-Konto auszuwählen, um die Verschlüsselung der Daten zu verwalten.
+

NOTE: Wenn Sie einen bestehenden Google Cloud Storage Bucket ausgewählt haben, sind die Verschlüsselungsinformationen bereits vorhanden, sodass Sie sie jetzt nicht eingeben müssen.
