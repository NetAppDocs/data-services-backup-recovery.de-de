...
---
= Cloud Volumes ONTAP kann mit NetApp Backup und Recovery in Google Cloud Storage gesichert werden
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
NetApp Backup und Recovery kann verwendet werden, um Volume-Daten von Ihren Cloud Volumes ONTAP Systemen in Google Cloud Storage zu sichern.
...
Sie benötigen außerdem ein Google Cloud-Konto für den Speicherplatz, auf dem Ihre Backups gespeichert werden.
...
Um die NetApp Backup und Recovery Search & Restore-Funktionalität zu nutzen, muss die Rolle für den Console-Agenten bestimmte Berechtigungen enthalten, damit auf den Google Cloud BigQuery Service zugegriffen werden kann. Die Berechtigungen sind unten aufgeführt, und die Schritte sind zu befolgen, falls eine Änderung der Richtlinie erforderlich ist.
...
+
.. *Projekt bearbeiten* auswählen und ein neues Projekt auswählen, falls das gewünschte Projekt vom Standardprojekt (in dem sich der NetApp Console Agent befindet) abweicht.
...
. *Dienste*: NetApp Backup und Recovery aktiviert lassen und *Weiter* auswählen.
...
.Schritte
. Auf der Seite *Systems* der NetApp Console das System auswählen und im rechten Bereich neben *Backup und Recovery* die Option *Enable* auswählen.
+
...
.. Unter *Zugriffsschlüssel für Dienstkonten* die Option *Schlüssel für ein Dienstkonto erstellen* auswählen, das soeben erstellte Dienstkonto auswählen und *Schlüssel erstellen* auswählen.
+
...
+
Wenn das Google Cloud Storage-Ziel für Ihre Backups als System auf der NetApp Console-Seite *Systems* vorhanden ist, kann der ONTAP Cluster auf das Google Cloud Storage-System gezogen werden.
...
** Konfigurieren Sie für Backup-to-Object-Richtlinien Datalock und Ransomware-Schutz. Weitere Informationen zu Datalock und Ransomware-Schutz finden Sie unter link:prev-ontap-policy-object-options.html["Einstellungen der Backup-to-Object-Richtlinie"].
...
+
Wenn Sie eigene, vom Kunden verwaltete Schlüssel verwenden, geben Sie die Schlüsselring- und Schlüsselinformationen ein.
