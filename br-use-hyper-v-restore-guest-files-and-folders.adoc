...
[role="lead"]
Stellen Sie Dateien und Ordner aus Hyper-V-VM-Backups auf dem primären Speicher in einer Windows-Gast-VM wieder her.
...
* Das Wiederherstellen von Dateien und Ordnern aus Sicherungen, die auf Sekundärspeicher oder Objektspeicher abgelegt sind, wird nicht unterstützt.
* Dateien oder Ordner können nicht auf VMs wiederhergestellt werden, die sich auf SAN-Speicher befinden.
...
.. Wählen Sie einen Snapshot auf dem primären Speicher aus, von dem die Wiederherstellung erfolgen soll, und klicken Sie dann auf *Weiter*.
