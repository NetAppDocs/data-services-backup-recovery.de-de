...
---
= Migration des Such- und Wiederherstellungskatalogspeichers zu ONTAP NFS
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Zur Verbesserung der Such- und Wiederherstellungsleistung in einer lokalen NetApp Console-Bereitstellung sollte der Katalogindexspeicher von der Standardspeicherklasse des Clusters auf eine von ONTAP unterstützte NFS-Freigabe migriert werden. Der Katalogindex unterstützt Such- und Wiederherstellung, und ONTAP-gestütztes NFS kann messbare Leistungsverbesserungen bieten.
...
[discrete]
===== Das ONTAP Volume auf der lokalen NetApp Console-Bereitstellung einbinden und Daten aus dem Pod kopieren
...
. Die folgenden PV- und PVC-YAML-Objekte werden erstellt:
+
**`pv.yaml`**
...
----
. Die YAML-Objekte werden angewendet, indem zuerst das PV und dann das PVC angewendet wird (das PV muss vorhanden sein, damit der Label-Selektor des PVC daran gebunden werden kann). Anschließend kann überprüft werden, ob der PVC-Claim gebunden ist:
+
[source, shell]
----
