...
---
= Kubernetes-Ressourcen während der Wiederherstellung in NetApp Backup and Recovery ändern
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Ressourcentransformationen ermöglichen die Modifikation von Kubernetes-Ressourcen während Wiederherstellungsvorgängen. Informationen zur Modifikation von Ressourcen in der Web-Benutzeroberfläche finden sich unter link:br-use-restore-kubernetes-applications.html#restore-kubernetes-applications["Ressourcen während der Wiederherstellung ändern"^].

*Erforderliche NetApp Console-Rolle* Backup and Recovery Superadministrator oder Backup and Recovery Restore-Administrator. Für den Schutz von Daten, die auf einem Cloud Volumes ONTAP Cluster gespeichert sind, ist zusätzlich die Cloud Volumes ONTAP Admin-Rolle erforderlich. link:reference-roles.html["Erfahren Sie mehr über die Zugriffsrollen für NetApp Backup and Recovery"]. https://docs.netapp.com/us-en/console-setup-admin/reference-iam-predefined-roles.html["Erfahren Sie mehr über die Zugriffsrollen der NetApp Console für alle Dienste"^].
...
Das folgende Beispiel zeigt die grundlegende Struktur eines Wiederherstellungsobjekts, das Ressourcentransformationen enthält:
...
* `group` (optional): Die API-Gruppe (z. B. `apps`, `route.openshift.io`) für Kernressourcen weglassen
...
Die  `replace`-Operation ersetzt einen vorhandenen Wert innerhalb einer Ressource am angegebenen Pfad. Der JSON Pointer-Pfad muss bereits existieren. Im folgenden Beispiel wird der Hostname eines Route-Objekts geändert:
