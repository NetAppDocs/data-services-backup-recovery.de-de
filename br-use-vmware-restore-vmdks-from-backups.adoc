...
---
= Wiederherstellung von VMware-virtuellen Festplatten aus Backups in NetApp Backup and Recovery
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Vorhandene, gelöschte oder getrennte virtuelle Festplatten (VMDKs) können aus primären oder sekundären Backups herkömmlicher VMs wiederhergestellt werden. Dadurch ist es möglich, bestimmte VM-Daten oder Anwendungen wiederherzustellen, ohne die gesamte VM wiederherstellen zu müssen. Nach der Wiederherstellung wird die virtuelle Festplatte an die ursprüngliche VM angehängt und ist einsatzbereit.
...
* Anfüge- und Wiederherstellungsvorgänge verbinden VMDKs über den standardmäßigen SCSI-Controller. Wenn jedoch VMDKs gesichert werden, die an eine VM mit einer NVMe-Festplatte angehängt sind, verwenden Anfüge- und Wiederherstellungsvorgänge einen NVMe-Controller, sofern einer verfügbar ist.
