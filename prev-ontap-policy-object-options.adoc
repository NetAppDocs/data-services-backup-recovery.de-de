...
Beide Funktionen schützen Ihre Sicherungsdateien, sodass Sie im Falle eines Ransomware-Angriffs stets über eine gültige Sicherungsdatei zur Datenwiederherstellung verfügen. Dies ist auch hilfreich, um bestimmte regulatorische Anforderungen zu erfüllen, bei denen Sicherungen für einen bestimmten Zeitraum gesperrt und aufbewahrt werden müssen. Wenn die Option *Datalock and Ransomware protection* aktiviert ist, wird für den im Rahmen der NetApp Backup and Recovery-Aktivierung bereitgestellten Cloud-Bucket die Objektsperrung und Objektversionierung aktiviert.
...
. Wenn Sie eine Richtlinie erstellen, erweitern Sie den Abschnitt *Datalock and Ransomware protection*.
