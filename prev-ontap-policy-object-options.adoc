...
[role="lead"]
Backup-to-Object-Richtlinien in NetApp Backup and Recovery ermöglichen die Definition von Zeitplänen, Aufbewahrungsfristen, DataLock, Ransomware-Schutz und Archivierungseinstellungen für On-Premises ONTAP und Cloud Volumes ONTAP Systeme.
...
Jede Sicherungsrichtlinie enthält einen Abschnitt für _Bezeichnungen & Aufbewahrung_, der auf die Sicherungsdateien angewendet werden kann. Es ist zu beachten, dass die auf das Volume angewendete Snapshot-Richtlinie eine der von NetApp Backup and Recovery erkannten Richtlinien sein muss, andernfalls werden keine Sicherungsdateien erstellt.
...
Beide Funktionen schützen Ihre Sicherungsdateien, sodass Sie im Falle eines Ransomware-Angriffs stets eine gültige Sicherungsdatei zur Datenwiederherstellung haben. Dies ist auch hilfreich, um bestimmte regulatorische Anforderungen zu erfüllen, bei denen Sicherungen gesperrt und für einen bestimmten Zeitraum aufbewahrt werden müssen. Wenn die Option *DataLock and Ransomware protection* aktiviert ist, werden für den im Rahmen der NetApp Backup und Recovery Aktivierung bereitgestellten Cloud-Bucket Objektsperrung und Objektversionierung aktiviert.
...
. Beim Erstellen einer Richtlinie lässt sich der Abschnitt *DataLock und Ransomware-Schutz* erweitern.
. Wählen Sie eine der folgenden Optionen:
+
** *Keine*: DataLock protection und Ransomware-Schutz sind deaktiviert.
** *Freigeschaltet*: DataLock-Schutz und Ransomware-Schutz sind aktiviert. Benutzer mit entsprechenden Berechtigungen können geschützte Sicherungsdateien während des Aufbewahrungszeitraums überschreiben oder löschen.
** *Gesperrt*: DataLock-Schutz und Ransomware-Schutz sind aktiviert. Keine Benutzer können während der Aufbewahrungsfrist geschützte Sicherungsdateien überschreiben oder löschen. Dies erfüllt die vollständige regulatorische Compliance.
...
| Cloud Volumes ONTAP in Google Cloud | Google Cloud-Speicher 
...
* Für AWS:
+
** Ihre Cluster müssen unter ONTAP 9.11.1 oder höher laufen
...
* Für Azure:
+
** Ihre Cluster müssen unter ONTAP 9.12.1 oder höher laufen
** Der Konsolenagent kann in der Cloud oder vor Ort eingesetzt werden
...
Darüber hinaus können Sie Schätzungen für die mit DataLock verbundenen Kosten erhalten, indem Sie die https://www.netapp.com/data-services/backup-and-recovery-tco-calculator/["Rechner für die Gesamtbetriebskosten (TCO) von NetApp Backup and Recovery"] .
...
Bei der Verwendung von AWS, Azure oder Google Cloud Storage können ältere Sicherungsdateien nach einer bestimmten Anzahl von Tagen in eine kostengünstigere Archivspeicherklasse oder Zugriffsebene verschoben werden. Es besteht auch die Möglichkeit, Sicherungsdateien unmittelbar in den Archivspeicher zu senden, ohne sie zuvor im Standard-Cloudspeicher abzulegen. Durch die Eingabe von *0* als „Archivierung nach Tagen“ wird die Sicherungsdatei direkt in den Archivspeicher verschoben. Dies kann besonders hilfreich für Benutzer sein, die nur selten auf Daten aus Cloud-Backups zugreifen oder eine Backup und Recovery auf Band ersetzen.
...
+
** Bei AWS können Sie Backups auf AWS _S3 Glacier_ oder _S3 Glacier Deep Archive_-Speicher auslagern. link:prev-reference-aws-archive-storage-tiers.html["Erfahren Sie mehr über AWS-Archivspeicher"^]Die
** Bei Azure können Sie ältere Backups im Azure-Archivspeicher auslagern. link:prev-reference-azure-archive-storage-tiers.html["Erfahren Sie mehr über Azure-Archivspeicher"^]Die
