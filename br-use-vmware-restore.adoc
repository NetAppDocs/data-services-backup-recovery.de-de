...
. Suchen Sie in der Liste die VM, die Sie wiederherstellen möchten, und wählen Sie für diese VM die Schaltfläche *Virtuelle Maschine wiederherstellen* aus.
+
Es wird eine Liste der auf dieser virtuellen Maschine erstellten Snapshots (Wiederherstellungspunkte) angezeigt.

. Aktivieren Sie das Kontrollkästchen für den Snapshot, den Sie wiederherstellen möchten.
. Wählen Sie *Weiter*.
+
...
*** *Datenspeicher*: Geben Sie den Namen des Datenspeichers ein, in dem Sie den Snapshot wiederherstellen möchten.
