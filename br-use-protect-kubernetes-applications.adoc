...
---
= Kubernetes-Anwendungen in NetApp Backup and Recovery hinzufügen und schützen
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Die NetApp Backup and Recovery Web-Oberfläche oder benutzerdefinierte Ressourcendateien ermöglichen das Hinzufügen von Kubernetes-Anwendungen zum Schutz. Namespace-basierte Anwendungen, die Standard-Kubernetes-Ressourcen verwenden, oder VM-basierte Anwendungen, die eine oder mehrere virtuelle Maschinen nutzen, können hinzugefügt werden.
...
Zum Schutz einer Kubernetes-Anwendung wird die Anwendung zunächst in NetApp Backup and Recovery erstellt. Beim Erstellen der Anwendung erkennt NetApp Backup and Recovery die laufende Anwendung im Kubernetes-Cluster.
...
. Oben rechts auf der Seite muss *Kubernetes* in der Liste der Workloads ausgewählt sein.
. Für den Workload-Eintrag kann *Ansicht* ausgewählt werden, um die Kubernetes-Ressourcen anzuzeigen.
...
NOTE: Backup and Recovery speichert weder die Suchparameter noch die Ergebnisse; die Parameter werden verwendet, um den ausgewählten Kubernetes-Cluster nach Ressourcen zu durchsuchen, die in die Anwendung aufgenommen werden können.
...
. Optional können Sie im Bereich *Richtlinie* eine vorhandene Datensicherungsstrategie auswählen, um die Anwendung zu schützen, oder eine neue Strategie erstellen. Wenn Sie keine Strategie auswählen, wird die Anwendung ohne Datensicherungsstrategie erstellt. Sie können link:br-use-policies-kubernetes-create.html#create-a-policy["Fügen Sie eine Schutzrichtlinie hinzu"] dies später nachholen.
...
NOTE: Wenn Sie VMs aus der Liste auswählen, ist die Anwendungsdefinition statisch, neue VMs werden der Anwendung nicht nachträglich hinzugefügt (die Anwendung muss bearbeitet werden, um sie hinzuzufügen und zu schützen). Wenn Label-Selektoren verwendet werden, können keine einzelnen VMs ausgewählt oder die generierte Liste bearbeitet werden, aber jede VM, die später dem Selektor entspricht, wird automatisch aufgenommen und geschützt.
...
.Ergebnis
Die Anwendung wird erstellt und erscheint in der Liste der Anwendungen auf der Registerkarte *Anwendungen* des Kubernetes-Inventars. NetApp Backup and Recovery ermöglicht den Schutz der Anwendung basierend auf Ihren Einstellungen, und der Fortschritt kann im Bereich *Monitoring* überwacht werden.
...
+
*** *metadata.annotations.protect.trident.netapp.io/skip-vm-freeze*: (_Optional_) Diese Annotation ist nur für Anwendungen relevant, die von virtuellen Maschinen aus definiert werden, z. B. in KubeVirt-Umgebungen, in denen das Dateisystem vor Snapshots eingefroren wird. Legen Sie fest, ob diese Anwendung während eines Snapshots auf das Dateisystem schreiben darf. Ist die Option auf true gesetzt, ignoriert die Anwendung die globale Einstellung und kann während eines Snapshots auf das Dateisystem schreiben. Ist die Option auf false gesetzt, ignoriert die Anwendung die globale Einstellung und das Dateisystem wird während eines Snapshots eingefroren. Wird die Option angegeben, die Anwendung aber keine virtuellen Maschinen in der Anwendungsdefinition hat, wird die Annotation ignoriert. Wird sie nicht angegeben, folgt die Anwendung der https://docs.netapp.com/us-en/trident/trident-protect/trident-protect-requirements.html#protecting-data-with-kubevirt-vms["Einstellung für das globale Dateisystem-Freeze"].
