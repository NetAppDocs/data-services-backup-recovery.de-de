...
---
= Kubernetes-Anwendungen mit benutzerdefinierten Ressourcen wiederherstellen
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Verwenden Sie benutzerdefinierte Ressourcen, um Ihre Anwendungen aus einem Snapshot oder einer Sicherung wiederherzustellen. Die Wiederherstellung aus einem vorhandenen Snapshot erfolgt in der Regel schneller, wenn die Anwendung im selben Cluster wiederhergestellt wird.
...
* Sie können die Wiederherstellungsvorgänge mithilfe erweiterter Einstellungen anpassen. Weitere Informationen finden sich unter link:br-use-kubernetes-advanced-restore-settings.html["Erweiterte Wiederherstellungseinstellungen für Kubernetes in NetApp Backup and Recovery"].
...
* Wenn eine CR zur Wiederherstellung in einem neuen Namespace verwendet wird, muss der Ziel-Namespace manuell erstellt werden, bevor die CR angewendet wird. Backup and Recovery erstellt Namespaces nur automatisch, wenn die CLI verwendet wird.
...
NOTE: Falls die BackupRestore CR nicht verfügbar ist, kann der Befehl in Schritt 2 verwendet werden, um den Sicherungsinhalt anzuzeigen.
